...
func (h *handler) UpdateWork(c *gin.Context) {
	...
	if err := tx.Where("work_id = ?", existingWork.ID).Delete(&Gallery{}).Error; err != nil {
		tx.Rollback()
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to delete old galleries: " + err.Error()})
		return
	}
	...
	if err := tx.Save(existingWork).Error; err != nil {
		tx.Rollback()
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to update work: " + err.Error()})
		return
	}
	...
	for _, gallery := range updatedWork.Gallery {
		...
		if err := tx.Create(&gallery).Error; err != nil {
			tx.Rollback()
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to create new galleries: " + err.Error()})
			return
		}
	}
	...
	if err := h.repo.db.Preload("Gallery").First(&existingWork, existingWork.ID).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to load updated work: " + err.Error()})
		return
	}
